...
private:
    ...
    void insert(const Thread * thr) { _threads.insert(new (SYSTEM) Queue::Element(thr)); }
    void remove(const Thread * thr) { Queue::Element * el = _threads.remove(thr); if(el) delete el; }
